...
public class ValidSudokuTest extends BaseTest {
  ...
  @DisplayName("Validate sudoku bitwise")
  @ParameterizedTest(name = "run #{index} should return [{1}]")
  @CsvSource(
    value = {
      "5,3,.,.,7,.,.,.,.;"
    ...
    delimiter = '|')
  public void isValidSudokuBitWise(final String input, final boolean expected) {
    //arrange
    var data = parseArrayOfArrayOfChar(input);
    //act
    boolean actual = ValidSudoku.isValidSudoku(data);
